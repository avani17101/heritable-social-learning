...
What should an artificial agent inherit? Avani Gupta's proposal here is to test
whether selection can discover a useful strategy for learning from others and
transmit that strategy to newborn agents. Each newborn faces a practical choice:
how much to trust its own noisy observations, how much to trust a peer, and when
the peer's information deserves skepticism.

The experiment separates inherited learning biases from acquired experience.
A compact genome shapes evidence weighting during learning; every generation
starts with fresh neural weights. The cycle is **inherited prior → lifetime
learning → performance → selection → next generation's prior**. This makes a
small version of the idea falsifiable before introducing richer developmental
genomes or cultural transmission.

The central test is whether an evolved prior improves newborn learning beyond
a strong, independently tuned fixed strategy. Useful transfer relative to random
genomes alone does not establish that evolutionary search is superior. The
lineage-permutation control also preserves population-level inheritance, so it
cannot identify a special causal role for parent–offspring continuity.

Read the separate [blog article, **What should an artificial agent inherit?**](blog/draft.md)
for the accessible narrative, and the [short launch post](blog/launch-post.md) for
a shareable introduction. **The blog was generated by OpenAI Codex from Avani
Gupta's research idea, project brief, and prototype results.** The README remains
the technical entry point; the blog is a separate document.
...
The current tuning protocol uses independent random streams for learning,
training-distribution screening, and validation. Their seeds are recorded in
`tuning.parquet`; baselines from the original screening protocol are rejected.
The initial run with reused screening trajectories is retained in
`results/archive_screen_reuse/`. The corrected rerun uses the same pre-specified
test seeds, so it is a bug correction rather than a new independent confirmation.
